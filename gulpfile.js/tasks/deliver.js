...
gulp.task('deliver', function(callback){
   ...
   */
  var copyTasks = ['deliver:copy:build', 'deliver:copy:sources'];

  if(config.copy.src_styleguide){
    copyTasks.push('deliver:copy:styleguide');
  }

  gulp.task('deliver:copy', copyTasks);
    ...
    if(config.copy.src_styleguide){
      gulp.task('deliver:copy:styleguide', function() {
        return gulp.src(config.copy.src_styleguide)
          ...
          .pipe(gulp.dest(config.copy.dest_styleguide));
      });
    }
